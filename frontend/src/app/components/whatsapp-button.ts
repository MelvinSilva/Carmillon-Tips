...
@Component({
  ...
  template: `
    <div class="fixed bottom-10 right-10 border p-4 rounded-full bg-white">
      <a
        href="{{ whatsappLink }}"
...
export class WhatsappButtonComponent implements OnInit, OnDestroy {
  constructor() {
    this.bounceSubscription = new Subscription();
  }
  private bounceSubscription: Subscription;
  public bounceState = 'initial';
  public isVisible = false;
  public whatsappLink: string = 'https://wa.me/33668908700';

  ngOnInit(): void {
    this.bounceSubscription = interval(2000).subscribe(() => {
      this.isVisible = true;
  ...
  handleClick(): void {
    window.open(this.whatsappLink, '_blank', 'width=600,height=600');
  }
}
